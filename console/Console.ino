...
// serving to log in to each node and to visualize to the user which
// nodes have booted.
...
// no error checking that takes place here: we presume that a
// node's name and IP address are returned well-formed (and if
// they are not, the user will see a visuailzation that something
