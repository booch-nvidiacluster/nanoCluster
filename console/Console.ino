...
// and IP address are discerned and its heartbeat, CPU utiliization, and
// memory utilization are queried reqularly.
...
// is data ready to be read from a given serial channel.
...
// byte from a given serial channel.
...
// address found therein. There is no error checking that takes
// place: the function assumes that the given string is well-formed,
// having derived as the response to a requestIP command sent across a
// serial channel to a given node. By well-formed, the function expects
// the IP address to reside between the first set of matched parenthesis
// in the given string.
...
// parses the value found therein. There is no error checking that takes
// place: the function assumes that the given string is well-formed,
...
// parses the value found therein. There is no error checking that takes
// place: the function assumes that the given string is well-formed, having
...
// is the string "login:". There is no error checking that takes place: we
...
// the end state being when it recognizes the final login symbol.
...
// Central qnd the TFT Touch Shield.
...
// There is no error checking that takes place: the function assumes that
// the name is well-formed (generally in the form "nodeClusterx", where x is
// the id of the node). Note that because this field in the display is also
// used to display the state of a node as it progresses through its lifetime,
// writing a new name involves re-writing the old name in the background
// color (effectively erasing it from view) and then writing the new name.
...
// node. There is no error checking place: the function assumes that
...
// that blinks with each change of state of a node's heartbeat.
...
// If it is discovered that a given node is in fact not powered on, it is 
...
// this phase, a node pushes out a long stream of output and this
// function looks for an indicator that the booting process has in
// fact completed. There is no error checking that takes place:
// if a node fails to boot, the set up process will hang.
...
// logginInNodes is the fifth function in the set up phase of each node,
// serving to log in to each node and then to visualize to the user which
// nodes have booted.
...
// confirmLoggedInNodes is the sixth and final function in the set
// up phase of each node, serving to request and then visualize
// to the user each node's name and its IP address. There is
// no error checking that takes place: we presume that a
// node's name and IP address are returned well-formed (and if
// they are not, the user will see a visualization that something
// went awry). Note that when getting a node's IP address, we
// have to throw away one line of the output to get to the proper
// information, and then we throw away the rest of the output.
...
// checkTouch is a function that is part of the console's continuous loop.
// It serves to sense if a user has touched a node's power icon, and if 
// the user has (and if the node is still powered on) it sends a
// command to shut down that node.
...
// listenToHeartbeat is a function that is part of the console's continuous
// loop. It serves to monitor each node's heartbeat, and when that
// hearbeat changes state, it toggles the dot in the display associated
// with that node, and it recalculates that node's CPU and memory
// utilization. Note that when getting a node's utilization, we
// have to throw away several lines of output to get to the proper
// information, and then we throw away the rest of the output.
...
// each node and its corresponding parts) and the journey of a simple state
// machine (that tracks the collective lifecyle of each node). There is no
// error checking that takes place: we assume that all nodes are
// well-behaved, and if they are not, set up will hang (with the user able
// to visualize exactly where the process failed by examining the display).
